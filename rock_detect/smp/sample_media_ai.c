...
#define AI_SAMPLE_CFG_FILE "./rock_detect.conf"
...
#define LANE_ID_SUBSCRIPT_3 3
    ...
    static HI_VOID RockClassifyAiProcess(VIDEO_FRAME_INFO_S frm, VO_LAYER voLayer, VO_CHN voChn)
    {
        int ret;
        if (GetCfgBool("rock_classify_switch:support_rock_classify", true))
        {
            if (g_workPlug.model == 0)
            {
                ret = Yolo2RockDetectResnetClassifyLoad(&g_workPlug.model);
                if (ret < 0)
                {
                    g_workPlug.model = 0;
                    SAMPLE_CHECK_EXPR_GOTO(ret < 0, ROCK_RELEASE,
                                           "load rock classify model err, ret=%#x\n", ret);
                }
            }

            VIDEO_FRAME_INFO_S resizeFrm;
            ret = MppFrmResize(&frm, &resizeFrm, ROCK_FRM_WIDTH, ROCK_FRM_HEIGHT);
            ret = Yolo2RockDetectResnetClassifyCal(g_workPlug.model, &resizeFrm, &frm);
            SAMPLE_CHECK_EXPR_GOTO(ret < 0, ROCK_RELEASE,
                                   "rock classify plug cal FAIL, ret=%#x\n", ret);

            ret = HI_MPI_VO_SendFrame(voLayer, voChn, &frm, 0);
            SAMPLE_CHECK_EXPR_GOTO(ret != HI_SUCCESS, ROCK_RELEASE,
    ...
    static HI_VOID *GetVpssChnFrameRockClassify(HI_VOID *arg)
    {
        int ret;
        ...
        while (HI_FALSE == g_bAiProcessStopSignal)
        {
            ...
            RockClassifyAiProcess(frm, voLayer, voChn);
        }
    ...
    static HI_S32 RockClassifyAiThreadProcess(HI_VOID)
    {
        HI_S32 s32Ret;
        ...
        s32Ret = pthread_create(&g_aiProcessThread, NULL, GetVpssChnFrameRockClassify, NULL);

        return s32Ret;
    }

    static HI_S32 PauseDoUnloadRockClassifyModel(HI_VOID)
    {
        HI_S32 s32Ret = HI_SUCCESS;
        ...
        if (GetCfgBool("rock_classify_switch:support_rock_classify", true))
        {
             ...
             */
            s32Ret = Yolo2RockDetectResnetClassifyUnload(g_workPlug.model);
            SAMPLE_CHECK_EXPR_RET(s32Ret != HI_SUCCESS, s32Ret, "unload rock model err:%x\n", s32Ret);
            ConfBaseExt();
            g_num = 0;
        }
    ...
    /*
     * 将sensor采集到数据显示到液晶屏上，同时创建线程运行岩石检测和分类推理计算
     * 视频输入->视频处理子系统->视频输出->显示屏
     *
     * Display the data collected by the sensor on the LCD screen,
     * and create threads to run rock detection and gesture classification reasoning calculations
     * VI->VPSS->VO->MIPI
     */
    HI_S32 SAMPLE_MEDIA_ROCK_CLASSIFY(HI_VOID)
    {
        HI_S32 s32Ret;
        HI_S32 fd = 0;
         ...
         */
        s32Ret = RockClassifyAiThreadProcess();
        ...
        g_aiProcessThread = 0;
        PauseDoUnloadRockClassifyModel();
