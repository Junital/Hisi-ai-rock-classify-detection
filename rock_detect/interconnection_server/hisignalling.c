...
static int GpioDirection(int pin, int dir)
{
    ...
    if (write(fd, &dirStr[dir == 0 ? 0 : 3], dir == 0 ? 2 : 3) < 0)
    { /* 3, 2, 3 gpio register */
...
static int GpioWrite(int pin, int value)
{
    ...
    if (write(fd, &valuesStr[value == 0 ? 0 : 1], 1) < 0)
    { /* 1, 1 gpio register */
...
void UartSendReadInt(int fd, int xmin, int ymin, int xmax, int ymax)
{
    unsigned char readBuff[16] = {0};
    unsigned char intbuffer[18] = {0, 6};

#ifdef EXPANSION_BOARD
    intbuffer[2] = xmin / 1000;
    xmin = xmin % 1000;
    intbuffer[3] = xmin / 100;
    xmin = xmin % 100;
    intbuffer[4] = xmin / 10;
    xmin = xmin % 10;
    intbuffer[5] = xmin;

    intbuffer[6] = ymin / 1000;
    ymin = ymin % 1000;
    intbuffer[7] = ymin / 100;
    ymin = ymin % 100;
    intbuffer[8] = ymin / 10;
    ymin = ymin % 10;
    intbuffer[9] = ymin;

    intbuffer[10] = xmax / 1000;
    xmax = xmax % 1000;
    intbuffer[11] = xmax / 100;
    xmax = xmax % 100;
    intbuffer[12] = xmax / 10;
    xmax = xmax % 10;
    intbuffer[13] = xmax;

    intbuffer[14] = ymax / 1000;
    ymax = ymax % 1000;
    intbuffer[15] = ymax / 100;
    ymax = ymax % 100;
    intbuffer[16] = ymax / 10;
    ymax = ymax % 10;
    intbuffer[17] = ymax;

    HisignallingMsgSend(fd, intbuffer, sizeof(intbuffer) / sizeof(intbuffer[0]));
    printf("send box size\r\n");
#endif
    /*
     * 串口读操作
     * Serial read operation
