...
#define AI_SAMPLE_CFG_FILE "./rock_classify.conf"
...
#define LANE_ID_SUBSCRIPT_3 3
    ...
    static HI_VOID RockClassifyAiProcess(VIDEO_FRAME_INFO_S frm)
    {
        int ret;
        if (GetCfgBool("rock_classify_switch:support_rock_classify", true))
        {
            if (g_workPlug.model == 0)
            {
                ...
                HI_ASSERT(g_aicMediaInfo.osds);
                ret = RockClassifyLoadModel(&g_workPlug.model, g_aicMediaInfo.osds);
                if (ret < 0)
                {
                    g_workPlug.model = 0;
                    SAMPLE_CHECK_EXPR_GOTO(ret < 0, ROCK_RELEASE,
                                           "load rock classify model err(%#x)\n", ret);
                }
            }
            VIDEO_FRAME_INFO_S *resFrm = NULL;
            ret = RockClassifyCal(g_workPlug.model, &frm, resFrm);
            SAMPLE_CHECK_EXPR_GOTO(ret < 0, ROCK_RELEASE,
    ...
    static HI_S32 RockAiThreadProcess(HI_VOID)
    {
        HI_S32 s32Ret;
    ...
    static HI_S32 PauseDoUnloadCnnModel(HI_VOID)
    {
        ...
        if (GetCfgBool("rock_classify_switch:support_rock_classify", true))
        {
             ...
             */
            s32Ret = RockClassifyUnloadModel(g_workPlug.model);
            SAMPLE_CHECK_EXPR_RET(s32Ret != HI_SUCCESS, s32Ret, "unload rock classify model err:%x\n", s32Ret);
            ConfBaseExt();
            g_num = 0;
     ...
     * and at the same time create a thread to run rock classification reasoning calculations
     * VI->VPSS->VO->MIPI
     */
